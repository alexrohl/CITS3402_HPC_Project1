...
CSR_Format get_CSR_CSC_multiply(MatrixContainer Matrix1, MatrixContainer Matrix2)
{
  ...
  if (Matrix1.isInt == 1) {
    ...
    //FLOATS
    //Float values
    float * fltNNZ_CSR = (float*)CSR_Matrix.NNZ;
    float * fltNNZ_CSC = (float*)CSC_Matrix.NNZ;
    for(i=0; i<Matrix1.n_rows; i++) {
      printf("row: %d\n",i);
      int row_first = CSR_Matrix.IA[i];
      int row_last = CSR_Matrix.IA[i+1];
      ...
      for(j=0; j<num_cols; j++) {
        ...
        while (true) { //multiplying row on column...
          ...
          //else CSC_matrix is spent, we are done as the rest is zeros
            ...
            //if indexs are the same
              ...
              cell_result += fltNNZ_CSR[i1] * fltNNZ_CSC[j1];
              //printf("row ind: %d, row val: %d, col ind: %d, col val: %d\n",i1,intNNZ_CSR[i1],j1,intNNZ_CSC[j1]);
              j1++;
              i1++;
            }
        ...
        if (cell_result == 0) {
          ...
          float_NNZ = append_float_to_array(float_temp, float_NNZ, cell_result, non_zero_counter);
          JA = append_int_to_array(temp, JA, j, non_zero_counter);
          non_zero_counter++;
  ...
  print_int_array(Result.IA, num_rows+1, "multIA");
  print_int_array(Result.JA, non_zero_counter, "multJA");
  print_int_array(Result.NNZ, non_zero_counter, "multNNZ");
