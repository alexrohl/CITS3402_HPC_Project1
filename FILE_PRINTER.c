...
FILE* CSR_Print(FILE* fp, CSR_Format CSR_Matrix, int n, int m, int isInt) {
  print_int_array(CSR_Matrix.IA, n+1, "inFILE_IA");
  print_int_array(CSR_Matrix.JA, CSR_Matrix.lenNNZ, "inFILE_JA");
  if (isInt==1) {
    print_int_array((int*)CSR_Matrix.NNZ, CSR_Matrix.lenNNZ, "inFILENNZ");
  } else {
    print_float_array((float*)CSR_Matrix.NNZ, CSR_Matrix.lenNNZ, "inFILENNZ");
  }
  int i,j;
  //printf("HERE");
  for (i=0;i<n;i++) {
